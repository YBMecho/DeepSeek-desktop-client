...
function makeSync() {
  ...
  function syncElectronTheme(explicitTheme, domRead) {
...
// 1. 点击"浅色"时 DOM 仍是旧的 system，必须写 light 而不是 system
{
  ...
  assert.deepStrictEqual(s.written, ['light'], '点击值必须优先于回读 DOM');
}
...
// 2. DOM 检测失败（选中类名不匹配）时不能兜底成 system
{
  ...
  assert.deepStrictEqual(s.written, [], '检测不到主题时不应写入任何值');
}
...
// 3. 相同主题重复点击只写一次
{
  ...
  assert.deepStrictEqual(s.written, ['light', 'dark'], '重复值应去重，变化值应写入');
}
...
// 4. 主进程推送触发的合成点击不回写（isApplyingThemeFromMain 同步清零）
{
  ...
  try { onClick('system'); } finally { isApplyingThemeFromMain = false; }
  assert.deepStrictEqual(s.written, [], '主进程推送的合成点击不应写回');
  onClick('light'); // 之后真实用户点击仍能生效
  assert.deepStrictEqual(s.written, ['light'], '清标志后用户点击应正常写入');
}
