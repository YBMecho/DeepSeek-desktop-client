 ...
 * 渲染进程 - 快捷键设置功能
 *
 * 功能：在设置面板中注入快捷键 / 悬浮窗快捷键 / 关闭行为 / 回复通知开关 /
 *       开机自启动等设置行，并与主进程同步主题与各项配置。
 */
...
(function() {
  ...
  let hotkeyInput: HTMLElement | null = null;
  let currentHotkey = 'Alt+`';
  let hotkeySelectContainer: HTMLElement | null = null; // 容器用于切换主题类
  let boundThemeSelectEl: HTMLElement | { value: string; addEventListener: () => void; removeEventListener: () => void } | null = null;
  let themeChangeHandler: ((event: Event) => void) | null = null;
  let mediaDark: MediaQueryList | null = null;
  let mediaDarkChangeHandler: (() => void) | null = null;
  // ponytail: 主进程推送"原生主题变化"事件的解绑句柄，cleanup 时调用避免重入泄漏。
  let unsubscribeNativeTheme: (() => void) | null = null;

  // 悬浮窗快捷键设置相关变量
  let isFloatingSettingMode = false;
  let floatingHotkeyInput: HTMLElement | null = null;
  let currentFloatingHotkey = 'Alt+Space';
  let floatingHotkeySelectContainer: HTMLElement | null = null;

  // 关闭行为设置相关变量
  let currentCloseBehavior = 'minimize';
  let closeBehaviorSelectContainer: HTMLElement | null = null;
  let closeBehaviorDisplay: HTMLElement | null = null;
  let closeBehaviorMenuWrapper: HTMLElement | null = null;
  let isCloseMenuOpen = false;

  // 回复通知开关设置相关变量
  let currentReplyNotifyEnabled = true;
  let replyNotifyToggleContainer: HTMLElement | null = null;
  let replyNotifyToggleInput: HTMLInputElement | null = null;

  // 开机自启动开关设置相关变量
  let currentAutoLaunch = true;
  let autoLaunchToggleContainer: HTMLElement | null = null;
  let autoLaunchToggleInput: HTMLInputElement | null = null;

  // 开机静默启动开关设置相关变量
  let currentSilentAutoLaunch = true;
  let silentAutoLaunchToggleContainer: HTMLElement | null = null;
  let silentAutoLaunchToggleInput: HTMLInputElement | null = null;

  // ponytail: 包裹"快捷键 + 关闭行为"两行的容器节点，用于跟随 tab 显隐。
  // 原网页切 tab 时整体面板不会被销毁，只会切 hidden/visible，所以我们注入
  // 的两行如果不显隐控制，就会跟着别的 tab 一起显示。
  let hotkeySectionWrapper: HTMLElement | null = null;
  let hotkeySectionParent: HTMLElement | null = null;
  ...
  // 将按键事件转换为快捷键字符串
  function keyEventToHotkeyString(event: KeyboardEvent): string {
  ...
  // 检查快捷键是否有效（至少包含一个修饰键）
  function isValidHotkey(hotkeyString: string): boolean {
  ...
  function findLanguageContainer(): HTMLElement | null {
    const allContainers = document.querySelectorAll<HTMLElement>('.ds-flex._50b3d9e');
    for (const container of allContainers) {
      if (container.textContent && container.textContent.includes('语言')) {
        return container;
      }
    }
    return null;
  ...
  function findThemeContainer(): HTMLElement | null {
    // 新版：按钮组一般包含在一个包裹"主题"文本的父容器内
    const groups = document.querySelectorAll<HTMLElement>('div._50e2ab4, div._0315fb1');
    ...
    const allContainers = document.querySelectorAll<HTMLElement>('.ds-flex._50b3d9e');
    for (const container of allContainers) {
      if (container.textContent && container.textContent.includes('主题')) return container;
    }
  ...
  // 移除已存在的快捷键设置
  function removeExistingHotkeySettings(): void {
    ...
    if (mediaDark && mediaDarkChangeHandler) {
      ...
      } else if ((mediaDark as unknown as { removeListener: (h: () => void) => void }).removeListener) {
        (mediaDark as unknown as { removeListener: (h: () => void) => void }).removeListener(mediaDarkChangeHandler);
      }
    }
    mediaDark = null;
    ...
    if (unsubscribeNativeTheme) {
      try { unsubscribeNativeTheme(); } catch (e) { /* ignore */ }
  ...
  function subscribeNativeThemeUpdates(): void {
  ...
  function createHotkeySettings(): void {
    // 首先检查是否为通用设置标签页
    ...
    hotkeySectionParent = languageContainer.parentNode as HTMLElement;
    ...
    const closeBehaviorContainer = hotkeyContainer.parentNode?.querySelector<HTMLElement>('.close-behavior-setting-flex');
    createFloatingHotkeySettings(closeBehaviorContainer || hotkeyContainer);

    // 创建悬浮窗重置设置（紧跟悬浮窗快捷键那一行）
    const floatingHotkeyContainer = hotkeyContainer.parentNode?.querySelector<HTMLElement>('.floating-hotkey-setting-flex');
    ...
    const floatingResetContainer = hotkeyContainer.parentNode?.querySelector<HTMLElement>('.floating-reset-setting-flex');
    ...
    const replyNotifyContainer = hotkeyContainer.parentNode?.querySelector<HTMLElement>('.reply-notify-setting-flex');
    ...
    // 创建开机静默启动开关（紧跟开机自启动开关那一行）
    const autoLaunchContainer = hotkeyContainer.parentNode?.querySelector<HTMLElement>('.auto-launch-setting-flex');
  ...
  function ensureHotkeySectionPlacement(): void {
  ...
  function getThemeSelectElement(): HTMLSelectElement | { value: string; addEventListener: () => void; removeEventListener: () => void } | null {
  ...
  // 通过多种方式判断当前网页主题
  function resolveThemeFromCssVar(): string | null {
    ...
    let el = document.querySelector<HTMLElement>('.ds-modal-content .ds-theme');
    if (!el) el = document.querySelector<HTMLElement>('.ds-theme');
  ...
  function applyHotkeyTheme(): void {
    // 新版本使用的是与语言选择框相同的样式，无需额外的主题适配
  }

  // 创建悬浮窗快捷键设置区域
  function createFloatingHotkeySettings(referenceContainer: HTMLElement): void {
  ...
  // 开始悬浮窗快捷键捕获
  function startFloatingHotkeyCapture(event: Event): void {
  ...
  // 捕获悬浮窗快捷键按下
  function captureFloatingKeyDown(event: KeyboardEvent): void {
    if (!isFloatingSettingMode) return;
  ...
  // 处理点击悬浮窗外部区域
  function handleFloatingOutsideClick(event: MouseEvent): void {
    if (!isFloatingSettingMode) return;

    if (floatingHotkeyInput && !floatingHotkeyInput.contains(event.target as Node)) {
      event.preventDefault();
      event.stopPropagation();
      exitFloatingSettingMode();
      ensureFloatingDisplayCorrectness();
  ...
  // 退出悬浮窗设置模式
  function exitFloatingSettingMode(): void {
    if (!isFloatingSettingMode) return;
  ...
  // 确保悬浮窗状态完全重置
  function ensureFloatingStateReset(): void {
    isFloatingSettingMode = false;
  ...
  // 确保悬浮窗显示内容和样式都正确
  function ensureFloatingDisplayCorrectness(): void {
    if (floatingHotkeyInput) {
      floatingHotkeyInput.classList.remove('setting-mode');
      floatingHotkeyInput.textContent = currentFloatingHotkey;
  ...
  // 更新悬浮窗快捷键显示
  function updateFloatingHotkeyDisplay(): void {
    if (floatingHotkeyInput) {
      floatingHotkeyInput.textContent = currentFloatingHotkey;
  ...
  // 恢复默认悬浮窗快捷键
  function restoreDefaultFloatingHotkey(): void {
  ...
  // 保存悬浮窗快捷键设置
  async function saveFloatingHotkeySetting(hotkey: string): Promise<void> {
    try {
  ...
  // 加载当前悬浮窗快捷键设置
  async function loadCurrentFloatingHotkey(): Promise<void> {
    try {
  ...
  function forceApplyTheme(isDark: boolean, source?: string): void {
    isApplyingThemeFromMain = true; // 设置标志，阻止 syncElectronTheme 响应

    // ponytail: 目标主题以 source 为准；没有 source 时只能按 isDark 猜（兼容旧调用）
    const targetTheme: string = (source === 'system' || source === 'light' || source === 'dark')
      ? source
      : (isDark ? 'dark' : 'light');
    ...
    try {
      ...
      for (const button of buttons) {
        ...
        if (!isSelected) (button as HTMLElement).click(); // click() 同步派发，返回时监听器已跑完
        return;
      }
  ...
  function forceApplyThemeDOM(isDark: boolean): void {
    ...
    const themeNodes = document.querySelectorAll<HTMLElement>('.ds-theme');
  ...
  function ensureSystemThemeWatcher(): void {
    ...
    if (isSystem) {
      ...
      if (mediaDark) {
        ...
        } else if ((mediaDark as unknown as { addListener: (h: () => void) => void }).addListener) {
          (mediaDark as unknown as { removeListener: (h: () => void) => void }).removeListener(mediaDarkChangeHandler);
          (mediaDark as unknown as { addListener: (h: () => void) => void }).addListener(mediaDarkChangeHandler);
        }
      }
  ...
  // 绑定主题选择器变化并立即应用一次
  function bindAndApplyTheme(): void {
    const selectEl = getThemeSelectElement();
    if (!selectEl) return;
    ...
    if (themeContainer) {
      ...
      buttons.forEach(button => {
        button.removeEventListener('click', themeChangeHandler as EventListener);
        button.addEventListener('click', themeChangeHandler as EventListener);
      });
    }
  ...
  function themeFromButtonText(el: Element | null): string | null {
  ...
  function getCurrentThemeFromDOM(): string | null {
    // 先尝试从新版本按钮获取主题
    const themeContainer = findThemeContainer();
    if (themeContainer) {
      const buttons = themeContainer.querySelectorAll('button, div[role="button"]');
  ...
  let lastSyncedTheme: string | null = null; // ponytail: 缓存上次同步的主题，避免重复写入
  let isCacheInitialized = false; // ponytail: 标记缓存是否已初始化

  // explicitTheme: 用户点击按钮时直接给出的主题值，优先于回读 DOM
  function syncElectronTheme(explicitTheme?: string | null): void {
  ...
  // 创建关闭行为设置区域
  function createCloseBehaviorSettings(hotkeyContainer: HTMLElement): void {
...
\tc-4.7-4.7-7-10.9-7-17c0-6.1,2.3-12.3,7-17c9.4-9.4,24.6-9.4,33.9,0L256,294.1z" fill="currentColor"></path>
  ...
  // 根据选择的值调整选择框宽度
  function adjustSelectWidth(event: { target: HTMLElement; value?: string }): void {
    const target = event.target;
    const container = target && target.closest ? target.closest('.close-behavior-select') as HTMLElement | null : closeBehaviorSelectContainer;
    if (!container) return;
  ...
  // 打开自定义下拉菜单
  function openCloseBehaviorMenu(): void {
    ...
    const buildOption = (value: string, text: string): HTMLElement => {
  ...
  function closeCloseBehaviorMenu(): void {
    if (!isCloseMenuOpen) return;
  ...
  function selectCloseBehavior(value: string): void {
  ...
  // 处理关闭行为选择变化
  async function handleCloseBehaviorChange(event: { target: { value: string } }): Promise<void> {
  ...
  // 创建回复完成系统提示音开关区域
  function createReplyNotifyToggle(referenceContainer: HTMLElement): void {
  ...
  // 创建开机自启动开关区域
  function createAutoLaunchToggle(referenceContainer: HTMLElement): void {
    const container = document.createElement('div');
  ...
  // 处理开机自启动开关切换
  async function handleAutoLaunchToggleChange(event: Event): Promise<void> {
    ...
    try {
      if (window.electronAPI && window.electronAPI.setAutoLaunch) {
        ...
        if (result && result.success) {
          ...
          target.checked = currentAutoLaunch;
        }
      } else {
        target.checked = currentAutoLaunch;
      }
  ...
  // 加载当前开机自启动开关
  async function loadCurrentAutoLaunch(): Promise<void> {
    try {
  ...
  // 创建开机静默启动开关区域
  function createSilentAutoLaunchToggle(referenceContainer: HTMLElement): void {
    const container = document.createElement('div');
  ...
  // 处理开机静默启动开关切换
  async function handleSilentAutoLaunchToggleChange(event: Event): Promise<void> {
    const target = event.target as HTMLInputElement;
    const enabled = !!target.checked;
    try {
      if (window.electronAPI && window.electronAPI.setSilentAutoLaunch) {
        ...
        if (result && result.success) {
          ...
          target.checked = currentSilentAutoLaunch;
        }
      } else {
        target.checked = currentSilentAutoLaunch;
      }
  ...
  // 加载当前开机静默启动开关
  async function loadCurrentSilentAutoLaunch(): Promise<void> {
    try {
  ...
  // 处理回复通知开关切换
  async function handleReplyNotifyToggleChange(event: Event): Promise<void> {
    const target = event.target as HTMLInputElement;
    const enabled = !!target.checked;
    try {
      if (window.electronAPI && window.electronAPI.setReplyNotifyEnabled) {
        ...
        if (result && result.success) {
          ...
          // 回滚 UI
          target.checked = currentReplyNotifyEnabled;
        }
      } else {
        target.checked = currentReplyNotifyEnabled;
      }
  ...
  // 加载当前回复通知开关
  async function loadCurrentReplyNotifyEnabled(): Promise<void> {
    try {
  ...
  // 加载当前关闭行为设置
  async function loadCurrentCloseBehavior(): Promise<void> {
    try {
  ...
  // 恢复默认快捷键
  function restoreDefaultHotkey(): void {
  ...
  // 开始快捷键捕获
  function startHotkeyCapture(event: Event): void {
    event.preventDefault();
    event.stopPropagation();
  ...
  function handleOutsideClick(event: MouseEvent): void {
    ...
    // 如果点击的不是快捷键输入框
    if (hotkeyInput && !hotkeyInput.contains(event.target as Node)) {
      event.preventDefault();
      event.stopPropagation();
      exitSettingMode();
  ...
  function ensureStateReset(): void {
    // 强制重置设置模式状态
    isSettingMode = false;
  ...
  // 确保显示内容和样式都正确
  function ensureDisplayCorrectness(): void {
    if (hotkeyInput) {
      // 移除所有可能影响显示的样式类
      hotkeyInput.classList.remove('setting-mode');
  ...
  // 保存快捷键设置
  async function saveHotkeySetting(hotkey: string): Promise<void> {
    try {
  ...
  // 加载当前快捷键设置
  async function loadCurrentHotkey(): Promise<void> {
    try {
  ...
  // 监听页面变化和标签页切换
  function observeSettingsModal(): void {
      ...
      mutations.forEach((mutation) => {
        mutation.addedNodes.forEach((node) => {
          if (node.nodeType === Node.ELEMENT_NODE) {
            ...
            // 检查是否是设置弹窗
            if (el.querySelector && el.querySelector('.ds-modal-content__title')) {
              const title = el.querySelector('.ds-modal-content__title');
              if (title && title.textContent && title.textContent.includes('系统设置')) {
            ...
            // 检查子节点中是否包含设置弹窗
            const settingsModal = el.querySelector('.ds-modal-content__title');
            if (settingsModal && settingsModal.textContent && settingsModal.textContent.includes('系统设置')) {
              createHotkeySettings();
              addTabClickListeners();
            }

            // 监听标签页选中状态变化
            if (el.classList && el.classList.contains('ds-segmented-button--selected')) {
              handleTabSwitch();
            }
          }
        });
    ...
    if (existingModal && existingModal.textContent && existingModal.textContent.includes('系统设置')) {
      createHotkeySettings();
      addTabClickListeners();
    }
  }
  ...
  // 监听body元素的主题变化
  function observeBodyThemeChanges(): void {
    const bodyElement = document.body || document.querySelector('body');
    if (!bodyElement) {
  ...
  // 添加标签切换/侧边菜单点击监听器
  function addTabClickListeners(): void {
    // 旧版顶部 tabs
    const tabs = document.querySelectorAll<HTMLElement>('.ds-segmented-button');
    tabs.forEach(tab => {
    ...
    // 新版左侧菜单按钮
    const sideButtons = Array.from(document.querySelectorAll<HTMLElement>('button, [role="button"]'))
