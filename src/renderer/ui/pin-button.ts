...
(() => {
  ...
  // 初始化：加载当前置顶状态
  async function initPinState(): Promise<void> {
    try {
  ...
  // 切换置顶状态
  async function togglePinState(): Promise<void> {
    try {
  ...
  // 更新所有按钮显示
  function updateAllButtonsDisplay(): void {
  ...
  // 在"新对话"按钮左侧添加置顶按钮
  function injectPinButtonBeforeNewChat(): boolean {
    // 查找"新对话"按钮 - 通过SVG路径特征匹配
    const allButtons = document.querySelectorAll<HTMLDivElement>('div[role="button"].ds-button--xl');
  ...
  function injectPinButtonInToolbar(): boolean {
    // 查找工具栏容器 .e5bf614e
    const toolbar = document.querySelector<HTMLDivElement>('.e5bf614e');
    ...
    // 查找第一个按钮（分屏按钮）
    const firstButton = toolbar.querySelector<HTMLDivElement>('div[role="button"].ds-button--m');
    if (firstButton) {
      const pinButton = createPinButton();
  ...
  // 主注入函数
  function injectPinButtons(): boolean {
    let injected = false;
  ...
  if (window.electronAPI && window.electronAPI.onFloatingWindowPinStateChanged) {
    window.electronAPI.onFloatingWindowPinStateChanged((pinned: boolean) => {
      isPinned = pinned;
      updateAllButtonsDisplay();
    });
  }
