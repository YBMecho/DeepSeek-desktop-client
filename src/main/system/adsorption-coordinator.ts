 ...
 */
function init(injectedDeps: Partial<CoordinatorDeps>) {
 ...
 */
function calculateDistance(rect1: { x: number; y: number; width: number; height: number }, rect2: { x: number; y: number; width: number; height: number }): number {
  // 计算水平方向的最短距离
  const dx = Math.max(0, 
 ...
 */
function setAdsorptionHighlight(text: string) {
 ...
 * @param {string} text - 提示文字
 */
function resetAdsorptionDefault(text: string) {
  const adsorptionWin = deps.getAdsorptionWindow();
  if (!adsorptionWin || adsorptionWin.isDestroyed()) return;
