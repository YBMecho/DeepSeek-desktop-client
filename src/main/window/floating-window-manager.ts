...
// 外部依赖（由 init 注入）
interface FloatingMgrDeps {
  getIsQuitting: () => boolean;
  applyWindowTheme: (win: BrowserWindow, isDark: boolean) => void;
  injectCustomAssets: (win: BrowserWindow, floatWin: BrowserWindow) => void;
  setupReinjectOnAuthNavigation: (win: BrowserWindow, floatWin: BrowserWindow) => void;
}

let deps: FloatingMgrDeps = {
  getIsQuitting: () => false,
  applyWindowTheme: () => {},
  injectCustomAssets: () => {},
 ...
 */
function init(injectedDeps: Partial<FloatingMgrDeps>) {
 ...
 */
function injectFloatingWindowDragArea(targetWindow: BrowserWindow) {
 ...
 * @param {string} option
 */
function getResetMinutes(option: string): number {
 ...
 * @param {string} hotkey
 */
function registerFloatingWindowHotkey(hotkey: string) {
  try {
...
function setFloatingResetOption(option: string) {
